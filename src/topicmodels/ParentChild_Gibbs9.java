...
public class ParentChild_Gibbs9 extends ParentChild_Gibbs_extra {
	public double m_mu;

	public ParentChild_Gibbs9(int number_of_iteration, double converge,
	...
	void sampleParentDocTopic(_ParentDoc2 d) {
		...
		for (int i = 0; i < d.m_words.length; i++) {
			...
			for (tid = 1; tid < number_of_topics; tid++) {
				probRatio[tid] = 0;
	...
	protected void estThetaInDoc(_Doc d) {
		if (d instanceof _ParentDoc2) {
			...
			for (int k = 0; k < number_of_topics; k++) {
				for (int x = 0; x < m_gamma.length; x++){
					double temp = Math.log(((_ChildDoc2) d).m_xTopics[x][k])+Math.log(((_ChildDoc2) d).m_xProportion[x]);
					d.m_topics[k] = Utils.logSum(d.m_topics[k], temp);
				}
				d.m_topics[k] = Math.exp(d.m_topics[k]);
			}
		}
